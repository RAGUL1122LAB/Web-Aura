...
export default function HeroSection() {
  ...
  return (
    <div className="border-3 border-gta-black bg-gta-beige text-gta-black gta-box-shadow select-none">
      ...
      <div className="relative w-full h-64 sm:h-80 bg-gta-orange overflow-hidden border-b-3 border-gta-black flex flex-col justify-between p-4 sm:p-6">
        ...
        <div className="relative z-10">
          <div className="inline-block bg-gta-black px-3 py-1 border-2 border-gta-orange mb-2">
            <span className="font-gta-condensed text-xs sm:text-sm font-black text-gta-orange uppercase tracking-widest">
              ANNUAL HACKATHON SHOWDOWN
            </span>
          </div>
          <h1 className="font-gta-title text-4xl sm:text-6xl text-white gta-text-shadow-lg leading-none tracking-tight flex items-center flex-wrap gap-2 sm:gap-3">
            <span>WEB</span>
            <span>AURA</span>
            <span className="text-white">2K26</span>
          </h1>
          <p className="font-gta-condensed text-lg sm:text-2xl font-black text-gta-black bg-gta-tan-light inline-block px-2.5 py-0.5 border-2 border-gta-black mt-2 uppercase tracking-wide">
            TWO ROUNDS • 100 TOTAL MARKS • LIVE CLOUD DEPLOYMENT
          </p>
        </div>

        {/* Call to Action Controls (Strict Rectangular, Black/Orange) */}
        <div className="relative z-10 flex items-center flex-wrap gap-3 pt-2">
          {currentUser ? (
            <Link to={role === 'EVALUATOR' ? '/evaluator' : role === 'ADMIN' ? '/admin' : '/dashboard'}>
              <SanAndreasButton variant="black" size="lg">
                ★ ENTER YOUR {role} HQ ★
              </SanAndreasButton>
            </Link>
          ) : (
            <>
              ...
              <Link to="/login">
                <SanAndreasButton variant="orange" size="lg">
                  [ PARTICIPANT LOGIN ]
                </SanAndreasButton>
              </Link>
            </>
          ...
          <Link to="/event">
            <SanAndreasButton variant="tan" size="md">
              [ VIEW RULES & MARKS ]
            </SanAndreasButton>
          </Link>
        </div>
